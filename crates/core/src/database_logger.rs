...
struct DatabaseLoggerWorker {
    logger: Arc<tokio::sync::Mutex<Box<dyn Logger + Send>>>,
    broadcast: broadcast::Sender<Bytes>,
}

impl DatabaseLoggerWorker {
    fn new(logger: impl Logger + Send + 'static, broadcast: broadcast::Sender<Bytes>) -> Self {
        let logger = Arc::new(tokio::sync::Mutex::new(Box::new(logger) as Box<dyn Logger + Send>));
